...
function GuestRoute({ children }) {
  ...
  if (ok)
    return (
      <Navigate to={loc.state?.from?.pathname ?? PATHS.DASHBOARD} replace />
    );
  return children;
}
...
const LoadCellPage = lazy(
  () => import("@/modules/devices/pages/LoadCellReportPage"),
);
...
export default function App() {
  return (
    <Suspense fallback={<PageLoader />}>
      <Routes>
        {/* Landing page lives at "/" — its Login button navigates to
            "/login" (same LandingPage component) to open the modal. */}
        <Route
          path="/"
          element={
            <GuestRoute>
