      ...
      className={({ isActive }) =>
        cn(
          ...
          isActive
            ...
            : 'text-sidebar-text hover:bg-sidebar-soft hover:text-[#dbe6f5]'
        )
      }
    >
      <Icon
        size={sub ? 16 : 18}
        strokeWidth={2.1}
        className="shrink-0 !text-[#CEA245]"
      />

      {!collapsed && <span className="truncate">{item.label}</span>}

      {!collapsed && item.badge != null && (
        <span className="ml-auto text-[10px] font-bold text-white px-1.5 py-0.5 rounded-md bg-red-500">
          {item.badge}
        </span>
      )}
