        ...
        <defs>
          {["v1", "v2", "v3", "v4"].map((k, i) => (
            <linearGradient key={k} id={`g${k}`} x1="0" y1="0" x2="0" y2="1">
              <stop
                offset="5%"
                stopColor={["#8b5cf6", "#10b981", "#f59e0b", "#f43f5e"][i]}
                stopOpacity={0.15}
              />
              <stop
                offset="95%"
...
function ChatbotWidget() {
  ...
  const sendImei = () => {
    ...
    setMsgs((p) => [
      ...
      {
        role: "bot",
        text: `IMEI **${imei}** identified. What would you like to do?`,
      },
    ]);
    setInput("");
  ...
  const selectOption = (opt) => {
    setMsgs((p) => [
      ...
      {
        role: "bot",
        text: `You selected **${opt}**. This conversation is now complete. You can close the widget.`,
      },
    ]);
    setStep(STEP.DONE);
    scroll();
          ...
          >
            ...
            <div className="flex items-center gap-3 px-4 py-3 bg-sidebar text-white">
              ...
              <button
                ...
                className="ml-auto text-sidebar-muted hover:text-white transition"
              >
                <X size={18} />
              </button>
            </div>
            ...
            <div className="p-3 border-t border-slate-100 flex gap-2">
              <input
                ...
                onKeyDown={(e) =>
                  e.key === "Enter" && step === STEP.IMEI && sendImei()
                }
                placeholder={
                  step === STEP.IMEI
                    ? "Enter IMEI…"
              ...
              <button
                onClick={sendImei}
                disabled={step !== STEP.IMEI || !input.trim()}
...
function exportCsv(data, imei) {
  ...
  const a = Object.assign(document.createElement("a"), {
    href: url,
    download: `LoadCellReport_${imei || "data"}_${Date.now()}.csv`,
  });
  a.click();
...
export default function LoadCellReportPage() {
  const accountId = useAccountStore((s) => s.selectedAccount?.id ?? 1);
  ...
  useEffect(() => {
    ...
    setImeiList([]);
    setImeiLoading(true);
    loadcellService.getImeis(accountId).then((list) => {
      setImeiList(list);
  ...
  const handleSearch = async (e) => {
    ...
    if (!from || !to) {
      setError("Please select From and To date-time.");
      return;
    }
    ...
    <div className="pb-20">
      ...
      <Card className="mb-5">
        ...
        <form onSubmit={handleSearch}>
          {error && (
            <div className="mb-4 px-3 py-2.5 rounded-xl bg-rose-50 border border-rose-100 text-sm text-rose-600 font-medium">
              {error}
            </div>
          )}
                ...
                {avgConfig && (
                  <div className="flex items-center gap-2 mt-1.5">
                    ...
                    <span className="text-xs text-slate-400">
                      Avg: {chartData[chartData.length - 1].Average.toFixed(2)}{" "}
                      tons
                    </span>
                  </div>
                )}
            ...
            <div className="mb-4">
              ...
              <div className="flex items-center gap-2 mt-1.5">
                <span className="text-xs font-bold text-violet-700 bg-violet-100 px-2 py-0.5 rounded-full">
                  Latest:{" "}
                  {chartData[chartData.length - 1].LoadPercent.toFixed(1)}%
                </span>
      ...
      {!searched && !loading && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
          {["Load Cell Graph with Averages", "Load Percentage (%)"].map(
            (title) => (
              <Card key={title}>
                ...
                <div className="flex items-center justify-center py-16">
                  <p className="text-sm text-slate-400 text-center max-w-xs">
                    Please select an IMEI and date range, then click Search to
                    load the chart.
                  </p>
                </div>
              </Card>
